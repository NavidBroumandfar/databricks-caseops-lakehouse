...
class DeltaShareConfig:
    ...
    shared_as
        Schema-qualified alias name for the table within the share. Databricks
        requires shared table aliases in `schema.name` form. Default:
        'gold.gold_ai_ready_assets'.
    ...
    @property
    def shared_delivery_events_as(self) -> str:
        """Schema-qualified alias for the shared delivery_events table."""
        return f"{self.gold_schema}.{self.delivery_events_table}"
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "shared_delivery_events_as": self.shared_delivery_events_as,
...
-- Prerequisite: run delivery_events_ddl from this manifest first so
-- caseops.gold.delivery_events exists before this share adds it.
...
ALTER SHARE {config.share_name}
  ...
  AS {config.shared_delivery_events_as}
...
CREATE TABLE IF NOT EXISTS {config.fully_qualified_delivery_events_table} (
  ...
  shared_object_name    STRING              COMMENT 'Shared table name. Default: gold.gold_ai_ready_assets.',
    ...
    return {
        ...
        "shared_tables": [
            ...
            },
            {
                ...
                "shared_as": config.shared_delivery_events_as,
                "description": (
...
def compute_c2_validation_queries(config: DeltaShareConfig) -> list[dict]:
    ...
    return [
        ...
        },
        {
            ...
            "description": (
                "Provider-side query of the Gold source table for export-ready "
                "records from a V2 batch. Shared-object visibility is validated "
                "by confirm_share_exists."
            ),
            "sql": (
                f"SELECT document_id, routing_label, schema_version "
                f"FROM {config.fully_qualified_gold_table} "
        ...
        },
        {
            ...
            "description": (
                "Confirm routing labels are visible in the Gold source table. "
                "Shared-object visibility is validated by confirm_share_exists."
            ),
            "sql": (
...
def compute_share_preparation_manifest(
    ...
    notes = [
        ...
        "To activate: run delivery_events_ddl first, then run setup_sql in a Databricks SQL notebook with CREATE SHARE privileges.",
...
def format_share_manifest_text(manifest: SharePreparationManifest) -> str:
    ...
    lines += [
        "",
        "=" * 70,
        "To provision: run delivery_events_ddl first, then run setup_sql in Databricks SQL.",
