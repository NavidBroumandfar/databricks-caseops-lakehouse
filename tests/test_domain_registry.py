...
Updated for D-2: incident_report is now ACTIVE. All three domains (FDA, CISA, incident)
are executable. No planned domains remain after D-2.
...
Covers:
    ...
    - Unregistered domain failure modes (clean errors, not silent)
    - extract_silver.select_extractor() domain-registry routing
    - classify_gold.select_classifier() domain-registry routing
    - All three active domains are fully runnable

Phase D-0/D-1/D-2 acceptance criteria alignment:
    ...
    - All three domains active after D-2
...
class TestRegistryIntegrity:
    ...
    def test_planned_domains_present(self):
        ...
        # D-2: incident_report graduated to ACTIVE — no planned domains remain
        assert len(planned_keys) == 0
...
class TestPlannedDomainFailures:
    ...
    D-2: all three domains are now ACTIVE. No planned domains remain.
    Parametrized tests that previously guarded planned incident_report are updated
    to verify D-2 graduation. Unregistered-domain error behavior is retained.
    """

    def test_no_planned_domains_after_d2(self):
        """D-2: all three reference domains are ACTIVE — planned list is empty."""
        planned = get_planned_domains()
        assert len(planned) == 0

    def test_incident_is_now_active_after_d2(self):
        """D-2: incident_report graduated from PLANNED to ACTIVE."""
        assert is_domain_active("incident_report") is True

    def test_incident_is_in_active_domains_after_d2(self):
        active_keys = [d.domain_key for d in get_active_domains()]
        assert "incident_report" in active_keys

    def test_incident_not_in_planned_list_after_d2(self):
        planned_keys = [d.domain_key for d in get_planned_domains()]
        assert "incident_report" not in planned_keys

    def test_incident_require_active_domain_does_not_raise(self):
        """D-2: require_active_domain no longer raises for incident_report."""
        domain = require_active_domain("incident_report")
        assert domain.status == DomainStatus.ACTIVE

    def test_incident_domain_status_is_active(self):
...
class TestPromptRouting:
    ...
    def test_incident_prompt_is_now_retrievable_after_d2(self):
        # D-2: incident_report has a registered prompt — all three domains active
...
class TestSchemaRegistry:
    ...
    def test_build_fields_for_incident_works_after_d2(self):
        # D-2: incident_report schema is now ACTIVE — build_fields_for_domain succeeds
    ...
    def test_get_schema_info_incident_active_after_d2(self):
        """D-2: get_schema_info on incident_report returns ACTIVE status."""
        info = get_schema_info("incident_report")
        assert info.status == DomainStatus.ACTIVE
        assert len(info.required_fields) > 0
...
class TestTaxonomyD0Extensions:
    ...
    def test_is_domain_executable_incident_now_true(self):
        # D-2: incident_report is now executable
    ...
    def test_resolve_routing_label_for_incident_resolves_after_d2(self):
        # D-2: incident_report resolves to incident_management
        result = resolve_routing_label_for_domain("incident_report", "incident_report")
        assert result == ROUTING_LABEL_INCIDENT_MANAGEMENT
...
class TestExtractSilverSelectExtractor:
    ...
    def test_incident_extractor_works_after_d2(self):
        # D-2: incident_report now has a real extractor
        from src.pipelines.extract_silver import select_extractor, LocalIncidentReportExtractor
...
class TestClassifyGoldSelectClassifier:
    ...
    def test_incident_classifier_works_after_d2(self):
        # D-2: incident_report now has a real classifier
        from src.pipelines.classify_gold import select_classifier, LocalIncidentReportClassifier
...
class TestAllThreeDomainsActiveAfterD2:
    """
    D-2: all three reference domains are ACTIVE and fully runnable.

    These tests verify that after D-2, incident_report joined FDA and CISA as a
    fully executable domain — extraction, schema construction, classification,
    prompt routing, and require_active_domain all succeed.

    Unregistered/unknown domains still raise cleanly (DomainNotFoundError /
    ValueError depending on the subsystem).
    """

    def test_all_three_active_in_domain_registry(self):
        active_keys = [d.domain_key for d in get_active_domains()]
        assert "fda_warning_letter" in active_keys
        assert "cisa_advisory" in active_keys
        assert "incident_report" in active_keys

    def test_incident_extraction_is_runnable(self):
        from src.pipelines.extract_silver import select_extractor
        extractor = select_extractor("incident_report")
        assert extractor is not None

    def test_incident_classification_is_runnable(self):
        from src.pipelines.classify_gold import select_classifier
        classifier = select_classifier("incident_report")
        assert classifier is not None

    def test_incident_schema_construction_is_runnable(self):
        model = build_fields_for_domain("incident_report", {})
        assert model is not None

    def test_incident_prompt_selection_is_runnable(self):
        prompt = get_prompt_for_domain("incident_report")
        assert prompt.document_domain == "incident_report"

    def test_incident_routing_is_runnable(self):
        result = resolve_routing_label_for_domain("incident_report", "incident_report")
        assert result == ROUTING_LABEL_INCIDENT_MANAGEMENT
    ...
    def test_incident_is_in_active_domains(self):
        active_keys = [d.domain_key for d in get_active_domains()]
        assert "incident_report" in active_keys

    def test_cisa_is_in_active_domains(self):
        active_keys = [d.domain_key for d in get_active_domains()]
        assert "cisa_advisory" in active_keys
