...
**C-1 implementation status**: All three v0.2.0 provenance fields are implemented in `ExportProvenance`. The `delivery_events` DDL is generated and ready to execute in Unity Catalog. No live Unity Catalog provisioning has been performed — that is a C-2 / manual step. All C-1 delivery events carry `status = 'prepared'`.
