...
Current version: **v0.2.0** (C-1 complete, April 2026; v0.1.0 = V1 baseline, still valid for V1 payloads)
...
**v0.2.0 additions** — **implemented in Phase C-1** (April 2026):

| Addition | Field | Location | Required | Implementation |
|---|---|---|---|---|
| Delivery mechanism identifier | `delivery_mechanism` | `export_payload.provenance` | Optional | `src/schemas/gold_schema.py` `ExportProvenance` |
| Delta Share name | `delta_share_name` | `export_payload.provenance` | Optional | `src/schemas/gold_schema.py` `ExportProvenance` |
| Delivery event reference | `delivery_event_id` | `export_payload.provenance` | Optional | `src/schemas/gold_schema.py` `ExportProvenance` |
...
v0.2.0 payloads are written when `--delivery-dir` is provided to `classify_gold.py`. V1 payloads (without `--delivery-dir`) continue to be written at v0.1.0. V1 test artifacts remain at v0.1.0 and are not retroactively updated.

A `caseops.gold.delivery_events` Delta table records per-batch delivery notifications. Its Pydantic schema is in `src/schemas/delivery_event.py`. Its SQL DDL is generated by `src/pipelines/delta_share_handoff.py`. The field specification is in [`docs/live-handoff-design.md`](./live-handoff-design.md) §10.

**C-1 implementation status**: All three v0.2.0 provenance fields are implemented in `ExportProvenance`. The `delivery_events` DDL is generated and ready to execute in Unity Catalog. Local repo execution does not provision Unity Catalog; Phase 3 later validated the producer-side share and delivery-events surface manually in a personal Databricks workspace on 2026-06-07. All C-1 delivery events carry `status = 'prepared'`.
